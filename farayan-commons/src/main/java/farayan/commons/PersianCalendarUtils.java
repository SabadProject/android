...
public class PersianCalendarUtils {
	...
	/**
	 * Converts a provided Persian (Shamsi) date to the Julian Dey Number (i.e.
	 ...
	 * Julian Dey Number is not as difficult as it looks. Basically it's a
	 * mather of dividing, rounding and multiplying. This routine uses Julian
	 * Dey Number 1948321 as focal point, since that Julian Dey Number
	...
	public static long persianToJulian(long year, int month, int day) {
		return 365L * ((ceil(year - 474L, 2820D) + 474L) - 1L) + ((long) Math.floor((682L * (ceil(year - 474L, 2820D) + 474L) - 110L) / 2816D)) + (PersianCalendarUtils.PERSIAN_EPOCH - 1L) + 1029983L
	...
	/**
	 * Converts a provided Julian Dey Number (i.e. the number of days since
