...
public class SabadUtility
{
	...
	public static CapturedBarcode CapturedBarcodeVersion(BarcodeResult barcodeResult) {
		return new CapturedBarcode(
				barcodeResult.getText(),
				BarcodeFormats.valueOf(barcodeResult.getBarcodeFormat().name()),
				barcodeResult.getBitmapScaleFactor(),
				barcodeResult.getBitmap(),
				Arrays.stream(barcodeResult.getResultPoints()).map(x -> new BarcodePoint(x.getX(), x.getY())).toArray(BarcodePoint[]::new)
		);
	}
	...
	public static PurchaseSummary PurchaseSummary(IGroupRepo groupRepo, IInvoiceItemRepo invoiceItemRepo) {
		...
		double quantitiesSum = itemedGroupes.stream().mapToDouble(x -> x.Item.Refreshed(invoiceItemRepo).Quantity).sum();
		double pricesSum = itemedGroupes.stream().mapToDouble(x -> x.Item.Refreshed(invoiceItemRepo).Total).sum();
