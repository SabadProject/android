	...
	private void Reload() {
		...
		RemainedCountEditText().setText(
				purchaseSummary.RemainedCount == 0
						...
						: getString(R.string.CheckoutPurchaseRemainedGroupsCount, (int) purchaseSummary.RemainedCount)
		);
		...
		return view -> {
			...
			for (GroupEntity groupEntity : pickedGroups) {
				if (groupEntity.Item != null) {
					...
					invoiceEntity.Total += groupEntity.Item.Total;
					invoiceEntity.ItemsQuantitySum += groupEntity.Item.Quantity;
					invoiceEntity.ItemsPriceSum += groupEntity.Item.Total;
